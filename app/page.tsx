...
export default function HomePage() {
  ...
  async function autoDetect() {
    setDetecting(true)
    try {
      const cached = sessionStorage.getItem('detectedState')
      if (cached) {
      ...
      if (location) {
        ...
        if (slug) {
          setDetectedState(slug)
          sessionStorage.setItem('detectedState', slug)
        }
      }
    } finally {
      setDetecting(false)
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white border border-accent rounded-lg p-4 mb-8 flex items-start gap-3 shadow-card">
