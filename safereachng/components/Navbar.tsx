...
import { AlertTriangle, Radio, Menu, X } from 'lucide-react'
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-navy border-b border-navy-light">
      <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3" onClick={() => setOpen(false)}>
          <Image src="/logo.png" alt="SafeReach NG" width={36} height={36} className="object-contain" />
          <div>
        ...
        <button onClick={() => setOpen(!open)} className="sm:hidden text-text-muted hover:text-white transition-colors p-2">
      ...
      {open && (
        <div className="sm:hidden bg-navy border-t border-navy-light px-4 py-3 space-y-1">
          <Link href="/feed" onClick={() => setOpen(false)} className={cn(
          ...
          <Link href="/report" onClick={() => setOpen(false)} className="flex items-center gap-2 px-3 py-2.5 text-sm font-medium rounded bg-danger/20 text-danger hover:bg-danger hover:text-white transition-colors">
            <AlertTriangle className="w-4 h-4" />
            Report an Incident
          </Link>
        </div>
